...
class RequestControllerTests {
  ...
  void testUpdateRequestDescription() {
    ...
    ResponseEntity<Request> response =
            requestController.updateRequestDescription(1, 1, "New Description");
    assertEquals(200, response.getStatusCodeValue());
    assertEquals(request, response.getBody());
  }
  ...
  void testUpdateRequestStatus() {
    when(requestService.updateRequestStatus(1, 1,
            CommonTypes.RequestStatus.approved)).thenReturn(request);
    ResponseEntity<Request> response = requestController.updateRequestStatus(1, 1,
